...
// Variable pour stocker les marqueurs des villes
...
inputCP.addEventListener("input", () => {
  ...
  // Efface les marqueurs des villes précédentes
      ...
      if (data.length > 0) {
        ...
        data.forEach((ville) => {
          ...
          if (
            ville &&
            ville.centre &&
            ville.centre.coordinates &&
            ville.centre.coordinates.length === 2
          ) {
            // Ajoute un marqueur pour la ville sur la carte
            L.marker([ville.centre.coordinates[1], ville.centre.coordinates[0]])
              .addTo(cityMarkers)
              .bindPopup(ville.nom);
          }
        });
